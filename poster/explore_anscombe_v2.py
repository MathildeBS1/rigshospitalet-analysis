...
# Unfiltered for bar chart stats
both_full = df.filter(
...
both = both_full.filter(pl.col("overrun_min").is_between(-60, 120))

all_vals = both["overrun_min"].to_numpy()
zero_prep = both.filter(pl.col("planned_prep_min") == 0)["overrun_min"].to_numpy()
has_prep = both.filter(pl.col("planned_prep_min") > 0)["overrun_min"].to_numpy()

frac_zero = len(zero_prep) / len(all_vals)
frac_has = len(has_prep) / len(all_vals)

x_grid = np.linspace(-60, 120, 500)
kde_all = gaussian_kde(all_vals, bw_method=0.15)
kde_zero = gaussian_kde(zero_prep, bw_method=0.15)
kde_has = gaussian_kde(has_prep, bw_method=0.15)

# --- Diverging bar data (from unfiltered) ---
buckets = [
...
for label, filt in buckets:
    ...
    labels.append(label)
    means.append(s["overrun_min"].mean())
...
# Use unfiltered means for annotations
mean_all_full = both_full["overrun_min"].mean()
mean_zero_full = both_full.filter(pl.col("planned_prep_min") == 0)["overrun_min"].mean()
mean_has_full = both_full.filter(pl.col("planned_prep_min") > 0)["overrun_min"].mean()
...
ax3.set_ylim(-16, 32)
ax3.set_ylabel("Mean overrun (min)", fontsize=10, color=MUTED)
...
for ax in (ax1, ax2):
    ax.set_xlabel("Overrun (minutes)", fontsize=10, color=MUTED)
...
plt.close()
print(f"Saved → {OUT}")
print(f"All: mean overrun={mean_all_full:.1f} min, n={total:,}")
print(f"Zero prep: mean overrun={mean_zero_full:.1f} min")
print(f"Has prep: mean overrun={mean_has_full:.1f} min")
